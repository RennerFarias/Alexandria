...
public class MenuFuncionario {
	public void exibirMenu() {
		String[] opcoes = { "Cadastrar Livro (Aquisição)", "Realizar Empréstimo", "Renovar Empréstimo",
				"Realizar Devolução", "Excluir Livro", "Gerar Relatórios / backup", "Listar Livros",
				"Listar emprestimos", "Sair" };
		...
		do {
			...
			Object[] mensagemMenu = { "Selecione a opção desejada:", menuDropdown };

			int acaoMenu = JOptionPane.showConfirmDialog(null, mensagemMenu, "Menu Principal - Biblioteca",
			...
			case 0:
				do {
					...
					Object[] formularioLivro = { "Titulo do Livro", campoTituloLivro, "Autor do Livro", campoAutor,
							"ISBN do Livro", campoIsbn, "Preco do Livro", campoPrecoCusto, "Quantidade de Estoque",
							campoQuantidadeDeEstoque, "Status", campoStatus, "Nota fistal", campoNotaFiscal };
					botaoClicado = JOptionPane.showConfirmDialog(null, formularioLivro, "Cadastro de livro",
							JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
					if (botaoClicado == JOptionPane.OK_OPTION) {
						try {
							...
							Livro novoLivro = new Livro(0, titulo, autor, isbn, precoCusto, quantidadeEstoque, status);

							DAO.LivroDAO livroDAO = new DAO.LivroDAO();
							livroDAO.cadastrarLivro(novoLivro);

							JOptionPane.showMessageDialog(null, "Livro cadastrado com sucesso!", "Operação Concluída",
							...
							JOptionPane.showMessageDialog(null, "Atenção: " + ex.getMessage(), "Regra de Negócio",
									JOptionPane.WARNING_MESSAGE);

						} catch (java.sql.SQLException e) {
							String mensagemAmigavel = TratamentoDeErros.obterMensagemAmigavel(e);

							JOptionPane.showMessageDialog(null, mensagemAmigavel, "Aviso do Sistema",
									JOptionPane.WARNING_MESSAGE);
						}
					} else {
						controleOpcao = false;
			...
			case 1:
				do {
					...
					Object[] formularioEmprestimo = { "Id do Usuario", campoIdUsuario, "Id do Livro", campoIdLivro };

					botaoClicado = JOptionPane.showConfirmDialog(null, formularioEmprestimo, "Emprestimo",
							JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
					...
					if (botaoClicado == JOptionPane.OK_OPTION) {
						try {
							...
							JOptionPane.showMessageDialog(null, "Empréstimo realizado com sucesso!",
									"Transação Concluída", JOptionPane.INFORMATION_MESSAGE);

							controleOpcao = false;

						} catch (NumberFormatException e) {
							JOptionPane.showMessageDialog(null, "Erro: Digite apenas números inteiros para os IDs.",
									"Erro de Digitação", JOptionPane.ERROR_MESSAGE);
			...
			case 2:
				do {
					String campoIdEmprestimo = JOptionPane.showInputDialog(null, "Id do Emprestimo", "Emprestimo",
							JOptionPane.QUESTION_MESSAGE);

					if (campoIdEmprestimo == null) {
						controleOpcao = false;
					} else if (campoIdEmprestimo.trim().isEmpty()) {
						JOptionPane.showMessageDialog(null, "O ID não pode ficar em branco!", "Aviso",
								JOptionPane.WARNING_MESSAGE);
					} else {
						try {
			...
			case 4:
				do {
					String inputIdLivro = JOptionPane.showInputDialog(null, "Id do livro", "livro",
							JOptionPane.QUESTION_MESSAGE);
					...
					if (inputIdLivro == null) {
						...
						try {
							int idLivro = Integer.parseInt(inputIdLivro.trim());

							DAO.LivroDAO livroDAO = new DAO.LivroDAO();
							// excluirLivro agora retorna boolean: true se excluiu, false se não achou o ID
							boolean excluiu = livroDAO.excluirLivro(idLivro);

							if (excluiu) {
								JOptionPane.showMessageDialog(null, "Livro excluído com sucesso!", "Exclusão Concluída",
										JOptionPane.INFORMATION_MESSAGE);
							} else {
							...
							JOptionPane.showMessageDialog(null,
									"Erro: Digite apenas números inteiros para o ID do livro.", "Erro de Digitação",
			...
			case 6:
				try {
					...
					JOptionPane.showMessageDialog(null, scrollPane, "Acervo da Biblioteca",
							JOptionPane.INFORMATION_MESSAGE);
			...
			case 7:
				try {
					...
					JOptionPane.showMessageDialog(null, scrollPane, "Lista de Empréstimos",
							JOptionPane.INFORMATION_MESSAGE);
